...
export default function App() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = auth.onAuthStateChanged((currentUser) => {
            setUser(currentUser);
            if (currentUser) {
                // FIX: Use `db.ref()` and `ref.on()` from the v8 SDK.
    ...
    useEffect(() => {
        if (originalImage && imageCanvasRef.current && maskCanvasRef.current) {
            ...
            // Set the canvas's internal (bitmap) size to match the image's aspect ratio.
            // We cap the largest dimension to 1024px for performance.
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (editorMode === 'photoshoot') {
                ...
                compositeCtx.drawImage(tempMaskCanvas, 0, 0, originalImage.width, originalImage.height);
            ...
            // FIX: Use `db.ref().update()` from the v8 SDK.
    ...
    const handleAuthAction = async (e: React.FormEvent) => {
        ...
        if (authView === 'signup') {
             try {
                // FIX: Use `auth.createUserWithEmailAndPassword` from the v8 SDK.
                const cred = await auth.createUserWithEmailAndPassword(email, password);
                // FIX: Use `db.ref().set()` from the v8 SDK.
            ...
            if (email === 'admin.opu@gmail.com' && password === 'opu@2525') {
                 try {
                    // FIX: Use `auth.signInWithEmailAndPassword` from the v8 SDK.
    ...
    const toggleAdminPanel = () => {
        if (showAdminPanel) {
            ...
            // FIX: To unsubscribe in v8, we must call `ref.off()`. Store a function that does this.
            usersListenerUnsubscribe.current = () => usersRef.off('value');
    ...
    // FIX: Filter out any invalid/null user entries from Firebase before mapping.
    // This prevents errors when spreading user data that might not be an object.
        ...
        <div className="min-h-screen bg-gray-950 text-gray-200 flex flex-col p-4 sm:p-6 lg:p-8 selection:bg-purple-500/30">
            ...
            <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col gap-4 bg-gray-900/50 p-6 rounded-2xl shadow-2xl shadow-purple-900/10 border border-gray-700">
                    ...
                    <div className="relative w-full flex-grow bg-gray-800/50 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-600 overflow-hidden">
                        {originalImage ? (
                            <div
                                className="relative"
                                style={{
                    ...
                    <div className="flex-grow bg-gray-800/50 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-600 overflow-hidden relative p-4">
                        {isLoading && (
                            <div className="flex flex-col items-center gap-2 text-gray-400">
                                ...
                                <p className="mt-2">AI is working its magic... this can take a moment.</p>
                            </div>
                        )}
